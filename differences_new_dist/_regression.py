...
from differences_new_dist.customDistMetric import CustomDistanceMetric
class LingerRegressorNewDist(BaseEstimator, RegressorMixin):
    def __init__(
        self,
        n_neighbours_1=5,
        n_neighbours_2=5,
        ...
        self.custom_distnace_metr = CVDHMDistanceMetric
    ...
    def fit(self, X, y):
        ...
        neighbours = NearestNeighbors(
            n_neighbors=self.n_neighbours_1,
            metric=CustomDistanceMetric(self.custom_distnace_metr.case_diff) # Use custom distance metric here
        ).fit(X)
        ...
        self.regressor.fit(differences_X, differences_y)
    ...
    def predict(self, X):
        ...
        nbrs = NearestNeighbors(
            n_neighbors=self.n_neighbours_2,
            metric=self.custom_distance_metric.case_diff  # Use your custom distance metric here
        ).fit(self.train_X)
        ...
        # The weighted knn version for regression.
        if self.weighted_knn:
            print("Weighted knn inplace")
            for indexes, differences, dist in zip(indices, predictions, distances):
                # Weights for each one. with 1e-8 to avoid division by 0 error.
                weights = 1 / (dist + 1e-8)
                # multiply its weight by its target value, for each of the neighbours.
                weighted_sum = sum(
                    w * (self.train_y[i] + d)
                    for i, d, w in zip(indexes, differences, weights)
                )
