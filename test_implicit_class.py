...
def train_linger_classifier(dev_X, dev_y, preprocessor, best_nn_params):
    ...
    lfd_gs = GridSearchCV(lfd_pipeline, lfd_param_grid, scoring="accuracy", cv=5, refit=True, n_jobs=8)
...
def main():
    ...
    print(f"Best Parameters Linger classifier: {lfd_gs.best_params_,}\n")
    print(f"Best Score Linger Classifier: {lfd_gs.best_score_}\n")
    print(f"Best Parameters Basic Neural Network: {nn_gs.best_params_,}\n")
    print(f"Best Score Basic Neural Network: {nn_gs.best_score_}\n")
