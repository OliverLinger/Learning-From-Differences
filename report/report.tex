...
\begin{enumerate}
	...
	\item \textbf{Susceptibility to Noise and Redundancy:} Instance-based approaches are sensitive to noisy or irrelevant features, which can lead to suboptimal performance or overfitting. 
    Redundant instances or outliers may distort similarity measures, compromising model robustness.
\end{enumerate}
...
Here is an image overview of the learning from differences process from the paper\cite{learningFromDifferences2022}.

\subsection{Image Depiction of \texttt{LingerRegressor}, \texttt{LingerClassifier}}
Here is the algorithm for the \texttt{LingerRegressor} in picture format. 
It is very similar for \texttt{LingerClassifier}, the difference being that instead of the average of 
predictions being taken at the end, a majority vote is taken.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{house_prices_training_part_1.PNG}
    \caption{\texttt{LingerRegression} training part 1}
    \label{fig:lingerRegression_training_part1}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{house_prices_training_part_2.PNG}
    \caption{\texttt{LingerRegression} training part 2}
    \label{fig:lingerRegression_training_part2}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{house_prices_training_part_3.PNG}
    \caption{\texttt{LingerRegression} training part 3}
    \label{fig:lingerRegression_training_part3}
\end{figure}


\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{house_prices_training_part_4.PNG}
    \caption{\texttt{LingerRegression} training part 4}
    \label{fig:lingerRegression_training_part4}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{house_prices_training_part_5.PNG}
    \caption{\texttt{LingerRegression} prediction part 1}
    \label{fig:lingerRegression_prediction_part1}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{house_prices_training_part_6.PNG}
    \caption{\texttt{LingerRegression} prediction part 2}
    \label{fig:lingerRegression_prediction_part2}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{house_prices_training_part_7.PNG}
    \caption{\texttt{LingerRegression} prediction part 3}
    \label{fig:lingerRegression_training_part6}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{house_prices_training_part_8.PNG}
    \caption{\texttt{LingerRegression} prediction part 4}
    \label{fig:lingerRegression_training_part7}
\end{figure}
...
During training, the Linger Regressor is fitted to the dataset $X$ with labels $y$. It computes nearest neighbors for each $X$
to construct features and targets by calculating differences between each sample and its neighbors, using scikit-learn's 
\texttt{NearestNeighbors}, \texttt{numpy}, and \texttt{MLPRegressor}. 
These differences train the regressor, and both the model and data are saved for future use.
\begin{algorithm}[H]
        ...
        \Function{predict}{$X$}
            ...
            \For{each sample $test$ in $X$}
                \For{each nearest neighbor $nn\_in\_training$ in $indices[test]$}
                    \State Append $(X[test, :] - train\_X[indices[test][nn\_in\_training], :])$ 
                    \State \hspace{\algorithmicindent} $\text{ to }differences\_test\_X$
                \EndFor
            \EndFor
            \If{$variant == 1$ (LRV1)}
        ...
        \Function{predict\_continue}{}
            ...
            \If{Weighted KNN is enabled}
                \For{each $indexes$, $differences$, $dist$ in $zip(indices, predictions, distances)$}
                    ...
                    \State $weighted\_sum = sum(w \times (train\_y[i] + d)$ 
                    \State \hspace{\algorithmicindent}for i, d, w in zip($indexes$, $differences$, $weights$))
...
\subsection{Objective}
The \texttt{LingerClassifier} extends the learning from differences methodology to classification tasks, aiming for:
\begin{itemize}
    \item Development of a flexible model that leverages differences between cases for predictions.
    \item Full compatibility with Scikit-learn, ensuring seamless integration and robust hyperparameter testing.
    \item Enhanced interpretability of predictions to improve user understanding.
    \item Continuous improvements in the classification performance through algorithmic enhancements.
    \item A modular and expandable design for accommodating future model variations.
\end{itemize}
This model is designed for classification scenarios demanding high interpretability and adaptability.

\subsection{Architecture}
The architecture of \texttt{LingerClassifier} parallels that of its regression counterpart, incorporating both K-nearest neighbors (KNN) and Scikit-learn's 
\texttt{MLPClassifier}. It utilizes KNN to determine nearest neighbors during both the training and prediction phases, adapting class labels based on 
learned differences.

\subsection{Components}
\texttt{LingerClassifier} comprises:
\begin{itemize}
    \item \textbf{KNN Module:} Determines nearest neighbors and captures case differences throughout the training and prediction processes.
    \item \textbf{\texttt{MLPClassifier}:} Allows for configurable activation functions and solvers, enhancing classification flexibility.
    \item \textbf{Supplementary Features:} Includes Weighted KNN, Context Addition, and Distance-Based Duplication among others, all configurable via hyperparameters.
\end{itemize}

\subsection{Parameterization}
Extensive parameterization options enable detailed customization of KNN and \texttt{MLPClassifier} settings, alongside specialized \texttt{LingerClassifier} parameters, facilitating precise model tuning for varied classification needs.
...
This algorithm implements the texttt{LingerClassifier} by fitting a Nearest Neighbors model, computing nearest neighbors for the training data, 
and calculating differences for feature preparation. Differences are used to train the \texttt{MLPClassifier}, accounting for sparse data and employing
variants like context addition or distance-based duplication as needed.
...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}[1]
        \Function{fit}{$X$, $y$}
            ...
            \If{$variant == 1$ (LCV1)}
                \State Call $addition\_of\_context\_func\_fit()$
                \State $differences\_X, differences\_y$ 
                \State \hspace{\algorithmicindent} $= addition\_of\_context\_func\_fit(X, y, indices, differences\_X, differences\_y)$
            \ElsIf{$variant == 2$ (LCV2)}
                \State Call $duplicate\_difference\_based\_on\_distance() = $
                \State $differences\_X, differences\_y = $ 
                \State \hspace{\algorithmicindent} $duplicate\_difference\_based\_on\_distance(differences\_X, differences\_y,$
                \State \hspace{\algorithmicindent} $distances)$
...
The prediction process utilizes the trained model and Nearest Neighbors to predict outcomes for new data. It includes adjustments for sparse data and 
leverages weighted KNN if enabled, ultimately aggregating predicted values based on computed differences.
...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}[1]
        \Function{predict}{$X$}
            ...
            \For{each sample $test$ in $X$}
                \For{each nearest neighbor $nn\_in\_training$ in $indices[test]$}
                    \State Append $(X[test, :] - \text{train\_X}[indices[test][nn\_in\_training], :])$ 
                    \State \hspace{\algorithmicindent} to $differences\_test\_X$
                \EndFor
            \EndFor
            \If{$variant == 1$ (LCV1)}
                \State Call $addition\_of\_context\_func\_pred(differences\_test\_X, X)$
...
Enhance model learning by including both the original attribute values and the differences with nearest neighbors in the dataset. 
This is applied similarly in both \texttt{LingerRegressor} and \texttt{LingerClassifier} models.

During both training and prediction, the algorithm appends the source case attributes to the differences array, 
enriching the feature set with contextual data. 
This is intended to improve prediction accuracy by preserving information about the original attributes.
\subsubsection{Training Design}
\begin{algorithm}[H]
...
\subsubsection{Objective}
This study introduces a variation that duplicates differences based on distance, enhancing the influence of neighbors closely resembling the base case. 
This novel approach diverges from traditional methods by prioritizing differences that strongly correlate with the base case, 
thus reducing the impact of outliers and enhancing model robustness. Integrated into the fitting process 
of both the \texttt{LingerClassifier} and \texttt{LingerRegressor}, this method emphasizes the importance of similar neighbors, 
improving the model's ability to identify meaningful patterns and boosting predictive accuracy and generalization.
...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}[1]
        \Function{$duplicate\_difference\_based\_on\_distance$}{$differences\_X,differences\_y,distances$}
            ...
            \Else
                \State $scaled\_distances$ $\gets 10 - ((log\_values - min\_val) / $
                \State \hspace{\algorithmicindent} $(max\_val - min\_val) \times 10)$
...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}[1] 
        \Function{duplicate\_diff\_contd}{$filtered\_data\_X, filtered\_data\_y,scaled\_dist\_rounded$}
            \State $duplicated\_list\_X \gets []$
            \State $duplicated\_list\_y \gets []$
            \For{each $item, count$ in zip($filtered\_data\_X$, $scaled\_dist\_rounded$)}
                \For{$\_ \gets 1$ to $count$}
                    \State append $item$ to $duplicated\_list\_X$
                \EndFor
            \EndFor
            \For{each $item, count$ in zip($filtered\_data\_y$, $scaled\_dist\_rounded$)}
                \For{$\_ \gets 1$ to $count$}
...
\subsubsection{Objective}
This novel variation incorporates the distance between each base case and its nearest neighbor into both the training and 
prediction phases of the neural network. By leveraging this proximity information, the approach enhances the learning process and predictive 
accuracy of the model. 
Integrating distance information helps the model capture nuanced patterns and relationships, enhancing its robustness and reliability. 
It also improves rhobustness by identifying outliers via distance.

\subsubsection{Design for Training}
In the training phase, distances between each base case $D^{train}_i$ and its nearest neighbor are calculated and stored in $Distances_{ij}$. 
These distances are then concatenated with the corresponding differences $\Delta D^{train}$, forming a new dataset where each difference is 
paired with its respective distance.
...
\subsubsection{Design for Prediction}
During the prediction phase, distances between each test case $X^{test}_j$ and its nearest neighbors are calculated and stored in $Distances_{ji}$. 
These distances are then added as attributes to the differences $\Delta X^{test}$, creating a dataset that maintains a one-to-one ratio between 
...
\subsection{Architecture}
The \texttt{LingerImplicitClassifier} combines K-nearest neighbors (KNN) and \texttt{MLPClassifier}, utilizing KNN for generating informative
 case pairs during both training and prediction phases. This hybrid approach leverages the strengths of both models to enhance classification accuracy.

\subsection{Components}
\begin{itemize}
    \item \textbf{KNN Module:} Identifies nearest neighbors to form case pairs.
    \item \textbf{\texttt{MLPClassifier}:} Provides a robust framework for learning from the identified case pairs.
    \item \textbf{Additional Features:} Includes class-to-class pair formation and selection mechanisms for neighbors.
\end{itemize}

\subsection{Parameterization}
The model provides extensive parameterization options for fine-tuning KNN, \texttt{MLPClassifier}, and additional features. 
Hyperparameters for the classification model mirror those available in standard KNN and neural networks, with additional parameters specific to
 \texttt{LingerClassifier}.
...
Both training and prediction phases share similar structural implementations, promoting code reuse and maintainability. 
The training phase involves preparing the neural networks by forming and utilizing case pairs, while the prediction phase focuses on applying 
these trained networks to new data.
    ...
    \begin{algorithmic}[1]
        \Function{predict}{$X$}
            ...
            \For{each $sample$ in $X$}
                \If{$variant == 2$ (LICV2)}
                    \State $class\_predictions \gets \text{ Predict class labels for } \text{ sample } \text{using }  1 \texttt{ NearestNeighbor}$
                \Else
                    \State $class\_predictions \gets \text{ Predict class labels for }  sample \text{ using }N \texttt{ NearestNeighbors}$
                \EndIf
                \State $majority\_prediction \gets \text{ Perform majority voting on } class\_predictions$
                \State $\text{Append }majority\_prediction \text{ to } y^{pred}$
            \EndFor
            \State \textbf{return} $y^{pred}$
        \EndFunction
    \end{algorithmic}
    ...
    \begin{algorithmic}[1]
        \Function{$\_predict\_class$}{$sample$, $nbrs\_by\_class$}
            ...
            \For{each $class\_label$, $nbrs$ in $nbrs\_by\_class.items()$}
                \If{$variant == 1$ (LICV1)}
                    \State $indices \gets \text{ Randomly select }n\_neighbours\_2 \text{ indices from } range(\text{len}(cases\_without\_concatenation[class\_label]))$
                \Else
                ...
                \For{each $neighbor\_index$ in $nearest\_neighbors$}
                    ...
                    \State $adapted\_prediction \gets$ Predict with $adaptation\_networks\_[class\_label]$ 
                    \State \hspace{\algorithmicindent} $\text{using }merged\_case\_pair$
...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}[1]
        \Function{$\_majority\_vote$}{$class\_predictions$}
            \State $all\_predictions \gets [prediction \text{ for predictions in }$
            \State \hspace{\algorithmicindent} $class\_predictions.values() \text{ for prediction in predictions}]$
...
\subsection{Components}
The components are analogous to those of the \texttt{LingerImplicitClassifier}, focusing on the effective 
generation of case pairs and leveraging neural networks for predictive modeling.
...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}[1]
    \Function{transformInit}{$y,min\_val, max\_val, unique\_ranges$}
        \State \textbf{Input:} 
        ...
        \State \textbf{Output:}
        \State $categories$ (list): A list of categories (segment indices) corresponding 
        \State \hspace{\algorithmicindent} to each value in $y$.
...
\begin{algorithm}
    ...
    \begin{algorithmic}
        \Function{generateDifferences}{$X$, $y$}
            ...
            \State \textbf{Output:} List of arrays representing differences $differences_X$, 
            \State \hspace{\algorithmicindent} List of target differences $differences_y$
...
\begin{algorithm}
    ...
    \Procedure{GenerateDifferences}{$X, y, n\_neighbours$}
        \State \textbf{Input:} $X \text{: dataset of images}, y\text{: target labels}, n\_neighbours \text{: number of neighbors}$
...
\begin{algorithm}
    ...
    \Procedure{Predict}{$X, model, input\_shape$}
        ...
        \State $differences\_test\_X \gets [np.abs(X[test].flatten() -$
        \State \hspace{\algorithmicindent} $self.train\_X[indices[test][nn]].flatten())$
        \State \hspace{\algorithmicindent} $\text{ for test in range}(len(X)) \text{ for nn in range}(len(indices[0]))]$
...
\begin{algorithm}
    ...
    \Procedure{Predict}{$X, model, dataset, input\_shape$}
        ...
        \State \textbf{Input:} $X \text{- Input data for prediction}, model \text{- Trained regression model},$
        \State \hspace{\algorithmicindent} $dataset \text{- Dataset object containing training data},$
        \State \hspace{\algorithmicindent} $input\_shape \text{- Shape of input data expected by the model}$.
        ...
        \State Identify nearest neighbors in the training set.
        ...
        \State Reshape $differences\_test\_X$ to match $input\_shape$ for 
        \State \hspace{\algorithmicindent} compatibility with the model.
...
\begin{table}[H]
    ...
    \midrule
    KNN & $91.7$ & $91$ \\
    Weighted KNN & $91.2$ & $\mathbf{91}$ \\
    Neural Network & $94.1$ & $\mathbf{91}$ \\
    LC (Base) & $91.7$ & $90.5$ \\
    LCV1 (Context) & $92.2$ & $89.5$ \\
    LCV2 (Duplicated on Distance) & $92.3$ & $90.5$ \\
    LCV3 (Additional Distance Column) & $91.7$ & $\mathbf{91}$ \\
    LIC (Implicit) & $91.6$ & $90.5$ \\
    LICV1 (Random) & $91.6$ & $89.5$ \\
    LICV2 (Single) & $91.5$ & $\mathbf{91}$ \\
    \bottomrule
    \end{tabular}
\end{table}
...
\begin{table}[H]
    ...
    LRV2 (Duplicated on Distance) & $-59.69$ & $\mathbf{-40.04}$ \\
...
\begin{table}[H]
    ...
    LC (Base) & $98.39$ & $\mathbf{100}$ \\
    LCV1 (Context) & $98.86$ & $100$ \\
    LCV2 (Duplicated on Distance) & $98.39$ & $\mathbf{100}$ \\
...
\subsection{Experiment 7: Image Classification using CIFAR-100}
\subsubsection{Methodology}
In this study, we utilized the CIFAR-100 dataset, ensuring data security through modified SSL settings. 
Our methodology focused on the development and comparative analysis of two convolutional neural network (CNN) models: 
the \textit{difference CNN model} and the \textit{standard CNN model}.

The \textit{difference CNN model} processes difference images created by the \texttt{LingerImageClassifier}, 
which uses settings of 5 neighbors for comparison to preprocess the original CIFAR images. 
This model's architecture, detailed in Table \ref{tab:cnn_models}, emphasizes feature extraction and computational efficiency, 
culminating in a regression output for predictions.

Simultaneously, the \textit{standard CNN model} was trained directly on normalized CIFAR-100 images, utilizing a similar 
architecture but concluding with a softmax layer for 100-class classification. Both models were trained over 10, 30, and 100 epochs, 
with a batch size of 32 and a 20\% validation split.

Post-training, model performance was evaluated based on accuracy, comparing predicted labels against the test set. 
This dual-model approach facilitates a thorough analysis of differing methodologies in image classification efficacy.

\begin{table}[H]
    \centering
    \caption{Comparison of CNN Architectures for Standard and Difference Models}
    \label{tab:cnn_models}
    \begin{tabular}{|c|c|c|}
    \hline
    \textbf{Layer Type} & \textbf{Standard Model} & \textbf{Difference Model} \\ \hline
    Input Layer & \multicolumn{2}{c|}{32x32x3 images} \\ \hline
    Conv2D & 32 filters, 3x3, padding=same, relu & 32 filters, 3x3, padding=same, relu \\ \hline
    Conv2D & 32 filters, 3x3, relu & 32 filters, 3x3, relu \\ \hline
    MaxPooling2D & 2x2 & 2x2 \\ \hline
    Dropout & 0.2 & 0.2 \\ \hline
    Conv2D & 64 filters, 3x3, padding=same, relu & 64 filters, 3x3, padding=same, relu \\ \hline
    Conv2D & 64 filters, 3x3, relu & 64 filters, 3x3, relu \\ \hline
    MaxPooling2D & 2x2 & 2x2 \\ \hline
    Dropout & 0.3 & 0.3 \\ \hline
    Conv2D & 128 filters, 3x3, padding=same, relu & 128 filters, 3x3, padding=same, relu \\ \hline
    Conv2D & 128 filters, 3x3, relu & 128 filters, 3x3, relu \\ \hline
    MaxPooling2D & 2x2 & 2x2 \\ \hline
    Dropout & 0.4 & 0.4 \\ \hline
    Flatten & \multicolumn{2}{c|}{Flatten output} \\ \hline
    Dense & 512, relu & 512, relu \\ \hline
    Dropout & 0.5 & 0.5 \\ \hline
    Output Layer & 100 classes, softmax & 1 output, linear \\ \hline
    \end{tabular}
\end{table}
\clearpage
...
\begin{table}[H]
    \centering
    \caption{Regression Performance on CIFAR-100 Dataset Over Different Epochs}
    \label{tab:regression_performance_cifar100}
    \begin{tabular}{|l|c|c|c|c|c|c|c|}
    \hline
    \textbf{Model} & \textbf{Epochs} & \textbf{MSE} & \textbf{Epochs} & \textbf{MSE} & \textbf{Epochs} & \textbf{MSE} \\ \hline
    LingerImageRegressor CNN & 10 & $0.0078$ & 30 & $0.0078$ & 100 & $0.0078$ \\ \hline
    Standard Regressor CNN & 10 & $0.0426$ & 30 & $\mathbf{0.0543}$ & 100 & $0.0486$ \\ \hline
    \end{tabular}
\end{table}

\subsubsection{Analysis}
The analysis of the LingerImageClassifier and Standard CNN on the CIFAR-100 dataset highlights the challenges of using a 
regression-based approach for classification tasks. While both models share similar convolutional architectures, they differ 
notably in their output layers. The Standard CNN is optimized for classification with a softmax layer, showing improved MSE from 0.0543 at 
30 epochs to 0.0486 at 100 epochs. Conversely, the LingerImageClassifier uses a linear output, typical for regression, to predict label 
differences, not direct classes, resulting in a low MSE of 0.0078 across all epochs. This consistently low MSE does not necessarily 
indicate high classification accuracy but rather the model’s precision in estimating label differences, which may not 
effectively reflect actual class distinctions. This misalignment suggests the need for future model adaptations that better 
suit classification by aligning output mechanisms with standard classification metrics and better addressing the categorical 
nature of the dataset. Future improvements might include a modified CNN that categorizes label differences directly, potentially 
involving a broader range of output classes. Which includes all possible differences.

\subsection{Experiment 8: Image Regression Using MNIST Dataset}
\subsubsection{Methodology}
In this proof-of-concept study, we adapted the MNIST dataset, traditionally used for classification, 
to evaluate image-based regression. We employed the \texttt{LingerImageRegressor} configured with two neighbors to 
manage image regression nuances effectively. The MNIST images were reshaped to 28x28x1, treating digit labels as 
continuous regression targets. We implemented and compared two CNN models: a standard CNN and a regression-specific 
modified CNN. Both were trained on original and transformed MNIST data, respectively—the latter enhanced by the \texttt{LingerImageRegressor} 
to highlight differences between neighboring images. This experiment primarily assessed the models' adaptability 
within a regression framework rather than focusing on achieving maximum accuracy, due to the dataset's inherent 
constraints for regression tasks.

\begin{table}[H]
    \centering
    \caption{CNN Architectures for LingerImageRegressor and Standard Regressor on MNIST}
    \label{tab:cnn_architecture}
    \begin{tabular}{|c|c|c|}
    \hline
    \textbf{Layer} & \textbf{LingerImageRegressor CNN} & \textbf{Standard Regressor CNN} \\ \hline
    Input Layer & \multicolumn{2}{c|}{28x28x1 images} \\ \hline
    Conv2D & 32 filters, 3x3, relu & 32 filters, 3x3, relu \\ \hline
    MaxPooling2D & 2x2 & 2x2 \\ \hline
    Conv2D & 64 filters, 3x3, relu & 64 filters, 3x3, relu \\ \hline
    MaxPooling2D & 2x2 & 2x2 \\ \hline
    Conv2D & 64 filters, 3x3, relu & 128 filters, 3x3, relu \\ \hline
    Flatten & \multicolumn{2}{c|}{Flatten output} \\ \hline
    Dense & 64, relu & 128, relu \\ \hline
    Output Layer & 1 output, linear & 1 output, linear \\ \hline
    \end{tabular}
\end{table}
\clearpage
...
\begin{table}[h]
    \centering
    \caption{Regression Performance on MNIST Dataset Over Different Epochs}
    \label{tab:regression_performance_epochs}
    \begin{tabular}{|l|c|c|c|c|}
    \hline
    \textbf{Model} & \textbf{Epochs} & \textbf{MSE at 10 Epochs} & \textbf{Epochs} & \textbf{MSE at 30 Epochs} \\ \hline
    LingerImageRegressor CNN & 10 & $7.2907$ & 30 & $7.3202$ \\ \hline
    Standard Regressor CNN & 10 & $0.1978$ & 30 & $\mathbf{0.1632}$ \\ \hline
    \end{tabular}
\end{table}
    
\subsubsection{Analysis}
The comparative analysis between the \texttt{LingerImageRegressor} and the Standard Regressor CNN on the 
MNIST dataset reveals that the \texttt{LingerImageRegressor}, designed to exploit differences between data points, 
produced higher mean squared errors (MSE) of 7.2907 and 7.3202 across 10 and 30 epochs respectively. 
This indicates a less effective adaptation to this particular dataset. Conversely, the Standard Regressor 
demonstrated substantially lower MSEs of 0.1978 and 0.1632, showing superior adaptation and learning efficiency. 
These results underscore the critical role of dataset suitability in machine learning, highlighting the MNIST 
dataset's limitations for regression tasks due to its categorical nature. Future experiments using a dataset with 
genuine continuous variables are recommended to better evaluate the \texttt{LingerImageRegressor}'s performance, 
enhancing the validity of this proof of concept.

\subsection{Comparative Analysis}
This section synthesises the findings of our experiments. It asses the blended learning approch and it performance overall.

\subsubsection{Key Findings}
\begin{itemize}
    \item \textbf{Performance Improvement:} Blended learning methods generally showed an improvement in accuracy and robustness across most datasets compared to traditional model-based and instance-based learning. 
    \item  For example, in experiments with the Glass and Abalone datasets, blended approaches significantly outperformed KNN and standard neural networks in terms of accuracy and handling noisy data.
    \item \textbf{Model Interpretability:} The experiments consistently demonstrated that blended learning approaches enhance model interpretability for classification tasks involving complex datasets compared to black-box models.
    \item \textbf{Efficiency and Scalability:} While blended models tend to require more computational resources initially, their adaptability to large datasets was evident, particularly in regression tasks with the House Prices and Abalone datasets
    \item \textbf{Innovative Architectures:} The exploration of novel architectures within the blended learning framework yielded promising results. Innovations such as case-based reasoning and feature difference techniques significantly enhanced model accuracy and interpretability, as shown in experiments 3 and 5.
    A specialised neural netowrk for the differences model could enhance ther performance even further, rather than resuing the trained neural networks from a seperate model.
    \item \textbf{Practical Applications:} Insights from the experiments indicate significant potential for blended learning methods in critical domains such as healthcare and finance. These methods are particularly beneficial in predictive analytics, where interpretability and accuracy are crucial.
\end{itemize}
\chapter{Conclusions and Future Work}
\label{ch:Conclusions and Future Work}
This chapter reflects on the contributions of our study, identifies its limitations, and outlines potential avenues for future research to 
extend the work's applicability and improve efficiency across multiple domains.

\section{Addressing Identified Limitations}
Our study, while providing valuable insights, recognizes certain limitations in scope and methodology. Future efforts should focus on 
surpassing these through advanced algorithmic enhancements and the integration of novel data processing techniques, such as those that 
could be implemented through Scikit-learn. These improvements are expected to refine the learning process, thus enhancing the models' 
overall performance and applicability.

\section{Innovative Label Representation}
A significant area for future research is the development of innovative methods for representing classification labels in ways that avoid 
implicit ordering. Traditional numeric and categorical encodings can inadvertently introduce bias or incorrect assumptions into the learning 
process, especially in models sensitive to feature scaling or order.This is true in the case of the \texttt{LingerClassifier}, which numerically encodes classes. 
Exploring label representations that maintain the distinctness of categories without implying ordinal relationships could enhance model accuracy and interpretability, particularly in complex classification tasks.
...
The potential integration of our models with emerging technologies, such as deep learning and reinforcement learning, offers exciting 
possibilities. Such integration could not only provide new insights and enhance model capabilities but also facilitate the use of these models in real-time applications or embedded systems where interpretability and efficiency are paramount.

\section{Scalability and Efficiency}
Investigating the scalability and efficiency of our models, especially in contexts involving large-scale datasets or distributed computing 
environments, is crucial. Future research could focus on optimizations that reduce training times or adaptations that leverage parallel 
processing architectures to improve performance.

\section{Cross-Domain Applications}
Examining the effectiveness of our models across different domains or for various types of prediction tasks could uncover new applications and 
opportunities for impact. This exploration might involve interdisciplinary research collaborations to adapt and evaluate our models in diverse 
contexts. It would be interesting to perform long term studies to asses the performance of blended learning models in dynamic enironments to validate their real world applications.

\section{Exploring Other Categorical Similarity Measures}
Investigating alternative measures for assessing the similarity of categorical features could refine the process of generating and applying 
adaptation rules, potentially enhancing the accuracy and applicability of the Ensemble of Adaptations for Classification (EAC). As seen in the 
paper \cite{jalali2017learning}. This could redefine the measurement of similarity between instances in a dataset.
